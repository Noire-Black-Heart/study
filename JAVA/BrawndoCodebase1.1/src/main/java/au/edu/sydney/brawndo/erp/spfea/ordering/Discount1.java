...
public class Discount1 implements Discount {
    ...
    private Order order;
    public Discount1(DiscountBean bean,Order order){
        this.bean = bean;
        this.order = order;
    }
    ...
    public double getTotalCost() {
        ...
        for (Product product: order.getAllProducts()) {
            cost +=  order.getProductQty(product) * product.getCost() * this.bean.getDiscountRate();
        }
        ...
        if(bean.isSubscription()){
            ...
            if(bean.isBusiness()){
                ...
                return String.format(false ? "" : "*NOT FINALISED*\n" +
                                ...
                                "Total cost: $%,.2f\n",
                        bean.getId(),
                        bean.getDate().format(DateTimeFormatter.ISO_LOCAL_DATE),
                        productSB.toString(),
