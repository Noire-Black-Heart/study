...
public class LastBlockSender implements Runnable {
	...
	public LastBlockSender(ConcurrentHashMap<ServerInfo, Date> serverStatus, Blockchain blockchain, int ownPort) {
	...
	public void run() {
		...
		while(true) {
			...
			//	Thread thread = new Thread(new LastBlockPrinter(serverInfo, "lb|" + ownPort + "|" + size + "|" + hash));
