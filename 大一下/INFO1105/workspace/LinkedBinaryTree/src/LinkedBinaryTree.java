...
public class LinkedBinaryTree<E> implements BinaryTree<E> {
	...
	public boolean isProper(Position<E> p) {
		...
		if(numChildren(current) == 1){
			...
			//boolean leftProper = 
			return		(isProper(current.getLeft()) && isProper(current.getRight()));
		}
		
//		for(Node<E> node : this){
//			if(numChildren(node) != 0 && !isExternal(node)){
//				return false;
//			}
//			return true;
//		}
...
//		return true;
	...
	public void reverse(Node<E> node, Node<E> revNode){
		if(node.getLeft() != null){
			revNode.setRight(new Node<E>(node.getLeft().getElement(), node.getParent(),null, null));
			reverse(node.getLeft(), revNode.getRight());
		}
		
		
		if(node.getRight() != null){
			revNode.setLeft(new Node<E>(node.getRight().getElement(), node.getParent(),null, null));
			reverse(node.getRight(), revNode.getLeft());
		}
...
//		Node<E> newleft = new Node<E>(this.root.getRight().getElement() , reversed.root, null, null);
//		Node<E> newright = new Node<E>(this.root.getLeft().getElement() , reversed.root, null, null);
//		reverse(this.root.getRight(), newleft);
//		reverse(this.root.getLeft(), newright);
//		reversed.root.setLeft(newleft);
//		reversed.root.setRight(newright);
		reverse(this.root, reversed.root);
		return reversed;
	}
	...
	public void reverseMMMe(Node<E> motherNode){
		if(motherNode == null){
			return;
		}
	Node<E> temp = motherNode.getRight();
    motherNode.setRight(motherNode.getLeft());
    motherNode.setLeft(temp);

    reverseMMMe(motherNode.getLeft());

    reverseMMMe(motherNode.getRight());
			//reverseMMMe(motherNode.getLeft()); 	
