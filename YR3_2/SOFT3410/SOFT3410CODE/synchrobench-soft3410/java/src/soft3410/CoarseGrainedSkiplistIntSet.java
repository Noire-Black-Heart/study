...
public class CoarseGrainedSkiplistIntSet extends AbstractCompositionalIntSet {
...
//---------------------------------------------------------------------------------------------
	...
	private synchronized void horizontalLink(Node node1, Node node2) {//1 is left
		node1.right = node2;
		node2.left = node1;
	}
	
	private synchronized void verticalLink(Node node1, Node node2) {//1 is higher
		node1.down = node2;
		node2.up = node1;
	}
    ...
    private synchronized Node findNode(int value){
        Node p = head;
        while(true){
        	//traverse the list from most top level
        	//System.out.println(p.right.getValue());
	...
	private synchronized void insertAfter(Node node, Node newNode) {//insert new node after node
		// TODO Auto-generated method stub
		newNode.left = node;
