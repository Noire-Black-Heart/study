...
public class CSVWriterTest {
	...
	public void test_empty_file() throws IOException {
		...
		assertThat(r.next(), is(SpecialBlocks.SKIP_BLOCK));
	...
	public void test_empty_lines() throws IOException {
		...
		for(int i=0; i<5; i++) {
			Block b = SpecialBlocks.EMPTY_BLOCK;
			w.write(b);
		}
		w.close();
	...
	public void test_incomplete_lines() throws Exception {
		...
		File incomplete = tmp.newFile();
		Writer w = CSVWriter.to(incomplete.getAbsolutePath());
		for(int i=1; i<8; i++) {
			SimpleBlock sb = new SimpleBlock();
			for(int j=1; j<4; j++) {
				sb.add("V"+j, "l"+i+"c"+j);
		...
		Reader csv = CSVReader.from(incomplete.getAbsolutePath());
		int numLines = 0;
		assertThat(csv.hasNext(), is(true));
		while(csv.hasNext()){
			Block b = csv.next();
